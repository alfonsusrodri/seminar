...
# ==========================
    ...
    penyakit_info = {
        ...
        "Two-Spotted Spider Mite": {
            "folder": "Spider_mites Two-spotted_spider_mite",
            "desc": "Hama tungau menyebabkan bintik kuning dan jaring halus."
        },
        "Yellow Leaf Curl Virus": {
            "folder": "Tomato_Yellow_Leaf_Curl_Virus",
    ...
    for i, (nama, info) in enumerate(penyakit_info.items()):
        folder = f"images/{info['folder']}"

        try:
            file_list = os.listdir(folder)
            img_path = os.path.join(folder, file_list[0]) if file_list else None
        except:
            img_path = None

        with outer_cols[i % 2]:

            # Kolom dalam: gambar & teks
            col1, col2 = st.columns([1, 2])

            with col1:
                if img_path:
                    st.image(img_path, width=120)  # kecil & rapi
                else:
